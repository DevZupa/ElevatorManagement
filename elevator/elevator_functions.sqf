...
ElevatorAddFriend = {
	...
	TheElevator setVariable ["elevatorfriends", _friends, true];
	PVDZE_veh_Update = [TheElevator,"gear"];
	publicVariableServer "PVDZE_veh_Update";
	if (isServer) then {
				PVDZE_veh_Update call server_updateObject;
	};
	call ElevatorGetFriends;
	call ElevatorNearbyHumans;
};

ElevatorRemoveFriend = {
	_pos = _this select 0;
	if (_pos < 0) exitWith {};
	_friends = TheElevator getVariable ["elevatorfriends", []];
	...
	TheElevator setVariable ["elevatorfriends", _newList, true];
	PVDZE_veh_Update = [TheElevator,"gear"];
	publicVariableServer "PVDZE_veh_Update";
	if (isServer) then {
				PVDZE_veh_Update call server_updateObject;
...
ElevatorGetFriends = {
	...
	{
	_counter = _counter +  1;
		lbAdd [7202, (_x select 1)];
	} forEach _friends;
};

ElevatorNearbyHumans = {
	lbClear 7201;
